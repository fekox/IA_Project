...
public class ECSExample_ECSWhitoutGOs : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        Parallel.For(0, entities.Count, i =>
        {
            ...
            RotationComponent rotationComponent = ECSManager.GetComponent<RotationComponent>(entities[i]); 
            drawMatrix[(i / MAX_OBJS_PER_DRAWCALL)][(i % MAX_OBJS_PER_DRAWCALL)]
            .SetTRS(new Vector3(position.X, position.Y, position.Z), Quaternion.Euler(rotationComponent.X,rotationComponent.Y,rotationComponent.Z), prefabScale);
        });
